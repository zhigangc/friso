...
__STATIC_API__ link_node_t new_node_entry( void * value, link_node_t prev, link_node_t next ) {
...
__EXTERN_API__ void free_link_list( friso_link_t link ) {
...
__EXTERN_API__ uint_t link_list_size( friso_link_t link ) {
	return link->size;
}

//check if the link list is empty
__EXTERN_API__ int link_list_empty( friso_link_t link ) {
	return ( link->size == 0 );
}
 ...
 * static
 */
__STATIC_API__ link_node_t get_node( friso_link_t link, uint_t idx ) {
 ...
 * static
 */
__STATIC_API__ void insert_before( friso_link_t link, link_node_t node, void * value ) {
 ...
 *		but we return it to you, you will free it youself when there is
 *		a necessary.
 *
 * @return the value of the removed node.
 */
__STATIC_API__ void *remove_node( friso_link_t link, link_node_t node ) {

	void *__value = node->value;
...
__EXTERN_API__ void link_list_add( friso_link_t link, void * value ) {
	insert_before( link, link->tail, value );
}

//add a new node before the given index.
__EXTERN_API__ void link_list_insert_before( friso_link_t link, uint_t idx, void * value  ) {
 ...
 */
__EXTERN_API__ void *link_list_get( friso_link_t link, uint_t idx ) {
	link_node_t node = get_node( link, idx );
	if ( node != NULL ) {
		return node->value;
	}
 ...
 */
__EXTERN_API__ void *link_list_set( friso_link_t link, uint_t idx, void * value ) {
 ...
 */
__EXTERN_API__ void *link_list_remove( friso_link_t link, uint_t idx ) {
 ...
 * @return the value of the node removed.
 */
__EXTERN_API__ void *link_list_remove_node( friso_link_t link, link_node_t node ) {
	return remove_node( link, node );
}
...
__EXTERN_API__ void link_list_add_last( friso_link_t link, void * value ) {
	insert_before( link, link->tail, value );
}

//append a note just after the head.
__EXTERN_API__ void link_list_add_frist( friso_link_t link, void * value ) {
